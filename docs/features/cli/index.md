...
| `--models-gateway <addr>`               | Route model traffic through a gateway. Also reads `DOCKER_AGENT_MODELS_GATEWAY` (legacy `CAGENT_MODELS_GATEWAY`) env var. When the gateway exposes an OpenAI-style `/v1/models` endpoint, the model picker defaults to the models actually advertised by the gateway (merged with the models defined in the agent config); press `tab` in the picker to toggle between gateway-advertised models and the full catalog. When the gateway doesn't expose `/v1/models`, the picker falls back to the regular catalog. |
